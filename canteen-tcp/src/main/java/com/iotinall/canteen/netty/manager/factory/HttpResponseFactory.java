...
public class HttpResponseFactory {
    ...
    public static final ConcurrentHashMap<Integer, ConcurrentHashMap<Integer, String>> KEEP_ALIVE_RESPONSE_MAP =
        ...
        try {
            ...
            if (null != temMap) {
                ...
                if (null != RESPONSE_DATA.putIfAbsent(iHashCodeKey, responseBody)) {
                    //出现多条响应入队的情况,记录错误日志
                    log.error("[KEEP_ALIVE][RESPONSE]-The {}- RESPONSE queue is full,this response will be drop. {}", iHashCodeKey, responseBody);
                }
                //如果已经有响应未提取,以后面的响应为准
                RESPONSE_DATA.put(iHashCodeKey, responseBody);
            }
        } catch (Exception e) {
            log.error("Handle response failed for {}", e.getMessage(), e);
        } finally {
        }
    }
